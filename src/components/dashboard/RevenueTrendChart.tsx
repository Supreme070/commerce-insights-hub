...
interface RevenueTrendChartProps {
  data?: { day: string; revenue: number; previous: number }[];
  period?: Period;
}
    ...
    return (
      <div className="bg-card rounded-xl p-3.5 shadow-level-3 border border-[hsl(var(--card-border))]">
        ...
        <div className="flex items-baseline gap-2">
          <p className="font-google text-sm font-semibold text-foreground">₦{current?.toLocaleString()}</p>
          {change && (
        ...
        <p className="text-[10px] text-text-muted mt-0.5">Previous: ₦{prev?.toLocaleString()}</p>
      </div>
    );
  }
  return null;
...
export function RevenueTrendChart({ data = [], period = 30 }: RevenueTrendChartProps) {
  const totalRevenue = data.reduce((sum, d) => sum + d.revenue, 0);
  const prevTotal = data.reduce((sum, d) => sum + d.previous, 0);
  const growthPct = prevTotal ? (((totalRevenue - prevTotal) / prevTotal) * 100).toFixed(1) : "0.0";
    ...
    >
      <div className="flex items-start justify-between mb-5">
        <div>
          ...
          <p className="text-xs text-text-muted mt-0.5">{periodLabel} vs previous period</p>
        </div>
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={data} margin={{ top: 4, right: 4, bottom: 0, left: 4 }}>
            ...
            <XAxis dataKey="day" tick={{ fontSize: 11, fill: "hsl(210, 5%, 63%)" }} axisLine={false} tickLine={false} interval={Math.max(Math.floor(data.length / 7), 1)} />
            <YAxis tick={{ fontSize: 11, fill: "hsl(210, 5%, 63%)" }} axisLine={false} tickLine={false} tickFormatter={(v) => `₦${(v / 1000).toFixed(0)}K`} width={55} />
            <Tooltip content={<CustomTooltip />} />
            <Area type="monotone" dataKey="previous" stroke="hsl(210, 5%, 80%)" strokeWidth={1.5} strokeDasharray="4 4" fill="none" dot={false} />
            <Area type="monotone" dataKey="revenue" stroke="hsl(214, 82%, 51%)" strokeWidth={2.5} fill="url(#blueGradient)" dot={false} activeDot={{ r: 5, stroke: "hsl(214, 82%, 51%)", strokeWidth: 2, fill: "white" }} />
          </AreaChart>
        </ResponsiveContainer>
      </div>
